...
- **补考闭环**：关考时未交卷记录自动记为缺考；缺考或成绩未达总分 60% 的学生可申请一次补考，教师批准后生成独立补考记录；报告、统计、排名与错题本均按两次考试的最高有效总分计算，原始记录仍可追溯；重复申请与并发批准只会成功一次。
...
| 方法 | 路径 | 说明 | 权限 |
| --- | --- | --- | --- |
| GET | `/api/v1/exams/:id/makeup` | 查询补考资格、审批状态与有效成绩 | 学生 |
| POST | `/api/v1/exams/:id/makeup` | 申请补考（缺考或低于总分 60%） | 学生 |
| GET | `/api/v1/exams/:id/makeup-requests` | 补考申请列表 | 管理员/教师 |
| POST | `/api/v1/makeup-requests/:id/approve` | 批准申请并生成补考记录 | 管理员/教师 |
| POST | `/api/v1/makeup-requests/:id/reject` | 拒绝申请 | 管理员/教师 |
